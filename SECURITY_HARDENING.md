...
| Sign-in CAPTCHA | With a site key configured the login and reset forms render Turnstile and forward the token (`captchaToken`) to Supabase Auth (`VITE_AUTH_CAPTCHA=false` hides it); submission is never blocked client-side. Enforcement happens in Supabase: Authentication → Attack protection → Enable CAPTCHA (Turnstile) with the matching secret | Code done; **Manual** to enforce |
