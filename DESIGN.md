...
|---|---|---|
| Global header | `src/components/Navbar.tsx` + tokens in `src/index.css` (`--header-logo`, `--header-py`, `--header-height`, `--header-transition`) | One header, two states: normal (desktop 93 px / 60 px logo, mobile 73 px / 52 px) and compact after 32 px of scroll (79 px / 54 px, mobile 63 px / 46 px); the emblem artwork is trimmed at build time so the box height is the visible mark. 200 ms padding/height transition, reduced-motion aware, hysteresis so it never flickers. Same nav, buttons and colours in both states; menu button stays 44 px. |
